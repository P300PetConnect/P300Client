...
import { ICoords } from './tracking.interface';
...
export class TrackingComponent implements OnInit {
  message: string;
  coords: ICoords[] = []; // Initialize coords as an empty array
  
  identityPoolId: string; 
  mapName = "location-service-2";
  credentials: any;
  geoLocation: any;
  mlglMap: maplibregl.Map;
  

  constructor(public datepipe: DatePipe) { 
    this.identityPoolId = "eu-west-1:9de2de22-a74d-4332-82a9-5658b0a9fcdf";
    AWS.config.region = this.identityPoolId.split(":")[0];
    this.credentials = new AWS.CognitoIdentityCredentials({
      IdentityPoolId: this.identityPoolId,
    });
    this.geoLocation = new AWS.Location({credentials: this.credentials, region: AWS.config.region});
